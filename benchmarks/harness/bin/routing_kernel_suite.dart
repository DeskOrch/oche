...
const _implementations = ['raw_dart_io', 'oche_tree', 'oche_indexed'];
const _workloadSets = <String, List<_Workload>>{
  'success': [
    _Workload('literal', 'GET', '/api/v1/status', 200),
    _Workload('single_parameter', 'GET', '/users/42', 200),
    _Workload('multiple_parameters', 'GET', '/users/42/orders/91', 200),
  ],
  'error': [
    _Workload('method_mismatch', 'DELETE', '/users', 405),
    _Workload('invalid_parameter', 'GET', '/users/abc', 400),
    _Workload('unexpected_exception', 'GET', '/errors/unexpected', 500),
  ],
};

Future<void> main(List<String> arguments) async {
  final options = _SuiteOptions.parse(arguments);
  final buildEntries = _readBuildEntries(options);
  final timestamp =
      options.suiteRunId ??
  ...
  for (final setName in options.workloadSets) {
    ...
    final resultsDirectory = Directory(
      '${options.resultsDirectory}/phase1a-$suiteRunId',
    );
        ...
        for (var iteration = 1; iteration <= options.iterations; iteration++) {
          ...
          for (
            ...
            for (
              ...
              final build = buildEntries.lookup(implementation, routeCount);
              trialSequence++;
              final output =
              ...
              if (completedTrialMatches(
                ...
                generatedSourceBytes: build.generatedSourceBytes,
                generatedSourceLines: build.generatedSourceLines,
                compileDurationMs: build.compileDurationMs,
                generatedSourceSha256: build.sourceSha256,
                binarySizeMb: options.mode == 'aot'
                    ? build.aotBinarySizeMb
                    : null,
                executableSha256: options.mode == 'aot'
              ...
              if (code != 0) {
                stderr.writeln(
                  'Routing suite stopped after benchmark exit code $code.',
                );
                exitCode = code;
                return;
              }
    ...
    final aggregateFile = File(
      '${resultsDirectory.path}/phase1a-$suiteRunId-aggregate.json',
    );
    await aggregateFile.writeAsString(
      '${const JsonEncoder.withIndent('  ').convert(aggregate)}\n',
    );
    stdout.writeln(
      'Wrote $setName aggregate for ${rawTrialFiles.length} raw trials to '
      '${aggregateFile.path} ($reusedTrials reused)',
    );
  }
}

bool completedTrialMatches(
  String path, {
  ...
  required int generatedSourceBytes,
  required int generatedSourceLines,
  required double? compileDurationMs,
  required String generatedSourceSha256,
  required double? binarySizeMb,
  ...
  try {
    ...
    return decoded['timestampUtc'] is String &&
        ...
        decoded['generatedSourceBytes'] == generatedSourceBytes &&
        decoded['generatedSourceLines'] == generatedSourceLines &&
        decoded['generatedSourceSha256'] == generatedSourceSha256 &&
        decoded['compileDurationMs'] == compileDurationMs &&
        decoded['binarySizeMb'] == binarySizeMb &&
        decoded['executableSha256'] == executableSha256;
  } on FormatException {
    return false;
...
_BuildEntries _readBuildEntries(_SuiteOptions options) {
  ...
  if (!manifest.existsSync()) {
    throw StateError(
      ...
      'Run tool/build-routing-kernels first.',
    );
  }
  final decoded = jsonDecode(manifest.readAsStringSync());
  if (decoded is! Map<String, Object?> ||
      decoded['entries'] is! List<Object?>) {
    throw const FormatException('Invalid routing-kernel build manifest.');
  }
  final entries = [
...
final class _BuildEntries {
  ...
  _BuildEntry lookup(String implementation, int routeCount) {
    if (entries.isEmpty) {
      final candidate = implementation.replaceFirst('oche_', '');
      final sourcePath = implementation == 'raw_dart_io'
          ? 'benchmarks/routing_kernel/bin/raw_server.dart'
          : 'benchmarks/routing_kernel/generated/'
                '${candidate}_$routeCount.dart';
      final source = File(sourcePath);
      return _BuildEntry(
        implementation: implementation,
        routeCount: implementation == 'raw_dart_io' ? null : routeCount,
    ...
    return entries.singleWhere(
      (entry) =>
          ...
          (implementation == 'raw_dart_io' || entry.routeCount == routeCount),
...
final class _SuiteOptions {
  ...
  static _SuiteOptions parse(List<String> arguments) {
    ...
    final workloadSets = (values['workload-sets'] ?? 'success,error')
        .split(',')
    ...
    if (workloadSets.isEmpty ||
      ...
      throw FormatException(
        '--workload-sets must contain success and/or error.',
      );
    }
